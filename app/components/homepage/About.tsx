...
import { Users, Mic, Building, Network, ArrowRight } from 'lucide-react'
...
const AboutSection: React.FC = () => {
  ...
  return (
    <section id="about" className="relative py-24 overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-50 to-indigo-100 transform -skew-y-6"></div>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <motion.h2 variants={itemVariants} className="text-4xl sm:text-5xl font-extrabold text-gray-900 mb-6">
        ...
        >
          {highlights.map((highlight, index) => (
            <motion.div
              ...
              className="relative"
            >
              <div className="absolute inset-0 bg-white shadow-lg rounded-2xl transform rotate-3 transition-transform duration-300 group-hover:rotate-0"></div>
              <div className="relative bg-white p-8 rounded-2xl shadow-md transform transition-transform duration-300 hover:-translate-y-2 hover:shadow-xl group">
                ...
                <p className="text-lg text-gray-600">{highlight.label}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
      ...
      <div className="absolute bottom-0 right-0 w-1/3 h-1/3 bg-gradient-to-tl from-indigo-200 to-transparent transform skew-x-12"></div>
    </section>
  )
}
